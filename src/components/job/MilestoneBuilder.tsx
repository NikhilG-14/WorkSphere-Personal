...
export default function MilestoneBuilder({
  ...
  const remaining = budget - totalMilestoneAmount;
    ...
    <div className="glass-panel p-6 rounded-xl border border-glass-border space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm">
          ...
          <span className={`font-semibold ${Math.abs(remaining) < 0.001 ? "text-success" : "text-warning"}`}>
            {totalMilestoneAmount.toFixed(2)} / {budget.toFixed(2)} SOL
          </span>
        </div>
      </div>
        ...
        <Alert className="border-warning/50 bg-warning/10">
          ...
          <AlertDescription className="flex items-center justify-between">
            <span className="text-warning">
              {remaining > 0 ? `${remaining.toFixed(2)} SOL remaining` : `${Math.abs(remaining).toFixed(2)} SOL over budget`}
            </span>
            <Button
              variant="outline"
              size="sm"
      ...
      <AnimatePresence mode="popLayout">
        {milestones.map((milestone, index) => (
          ...
          >
            <div className="flex items-center justify-between">
              <h3 className="font-semibold text-neon-gold">
                Milestone {index + 1}
              </h3>
              <Button
                variant="ghost"
